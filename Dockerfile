...
RUN dotnet build "ConfirmCoords.API.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "ConfirmCoords.API.csproj" -c Release -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "ConfirmCoords.API.dll"]